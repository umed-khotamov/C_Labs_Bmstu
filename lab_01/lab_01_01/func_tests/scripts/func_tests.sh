...
while [ $index_1 -ne ${#array_pos_in[*]} ]; do
    ./pos_case.sh "../data/${array_pos_in[$index_1]}" "../data/${array_pos_out[$index_1]}" 
    return=$?
    if [ $return -eq 0 ]; then
...
while [ $index_2 -ne ${#array_neg_in[*]} ]; do
    ./neg_case.sh "../data/${array_neg_in[$index_2]}"
    return=$?
    if [ $return -eq 0 ]; then
...
if [ "$1" = "--USE_VALGRIND" ]; then
    ...
    index_2=0
    while [ $index_1 -ne ${#array_pos_in[*]} ]; do
        ./pos_case.sh "../data/${array_pos_in[$index_1]}" "../data/${array_pos_out[$index_1]}" "$command_valgrind"
    ...
    while [ $index_2 -ne ${#array_neg_in[*]} ]; do
        ./neg_case.sh "../data/${array_neg_in[$index_2]}" "$command_valgrind"
        if [ -s report.txt ]; then
